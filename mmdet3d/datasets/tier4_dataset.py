...
from .nuscenes_utils import nuScenesDetectionEval


@DATASETS.register_module()
class T4Dataset(Custom3DDataset):
    r"""T4 Dataset.
    ...
    def _evaluate_single(self,
        ...
        #得到gt_dict 和 pred_dict用来eval
        result_dict = mmcv.load(result_path)['results']

        tmp_dir = tempfile.TemporaryDirectory()
        assert tmp_dir is not None

        gt_dict = mmcv.load(self._format_gt_to_nusc(tmp_dir.name))['results']
        tmp_dir.cleanup()
    ...
    def _format_gt_to_nusc(self, output_dir, pipeline=None):
        ...
        for sample_id in range(len(self.data_infos)):
            ...
            gt_names = self.data_infos[sample_id]['gt_names'].tolist()#有些name不在CLASSES中
            gt_velocity = self.data_infos[sample_id]['gt_velocity']

            nus_lidar_box_list = gtbox_to_nusc_box(gt_boxes, gt_velocity)
            nus_global_box_list = gt_lidar_nusc_box_to_global(self.data_infos[sample_id],nus_lidar_box_list)
            annos = []
            
            for box, name in zip(nus_global_box_list, gt_names):
                ...
                annos.append(nusc_anno)
            all_annotations[sample_token] = annos
        nusc_submissions = {
            'meta': self.modality,
...
def gtbox_to_nusc_box(boxes, velocitys):
    box_list = []
    for i in range(len(boxes)):
        center = boxes[i][:3]
        wlh = boxes[i][[4, 3, 5]]
        box_yaw = boxes[i][6]
        box_vel = velocitys[i].tolist()
        box_vel.append(0)#add z
        quat = pyquaternion.Quaternion(axis=[0, 0, 1], radians=box_yaw)
        nusc_box = NuScenesBox(center, wlh, quat, velocity=box_vel)
        box_list.append(nusc_box)
    return box_list

def gt_lidar_nusc_box_to_global(info, boxes):
    lidar2ego_t = np.array(info['lidar2ego_translation'])
    ...
    box_list = []
    for box in boxes:#tier4数据集ego与lidar在一个坐标系
        # move box to ego coord system
        box.rotate(pyquaternion.Quaternion(lidar2ego_r))
        box.translate(lidar2ego_t)
        # move box to global coord system
        box.rotate(pyquaternion.Quaternion(ego2global_r))
