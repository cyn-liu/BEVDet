...
mmlab/OpenPCDet/blob/v0.5.2/pcdet/datasets/nuscenes/nuscenes_utils.py."""
import operator
...
def transform_det_annos_to_nusc_annos(det_annos, velocity_threshold=0.2):
    """convert detections to nuscenes-format for evaluation modified from:
    https://github.com/open-
    mmlab/OpenPCDet/blob/master/pcdet/datasets/nuscenes/nuscenes_utils.py#L423.
    ...
    for det in det_annos:
        ...
        for k, box in enumerate(box_list):
            name = det['name'][k]
            if np.sqrt(box.velocity[0]**2 +
                       box.velocity[1]**2) > velocity_threshold:
                if name in [
...
def format_nuscenes_metrics(metrics: Dict,
...
def format_nuscenes_metrics_table(metrics, class_names, logger=None):
    ...
    # not use error@attr
    errors = [
        x.split('_')[0] for x in list(metrics['label_tp_errors'][name].keys())
    ][:4]

    APs_data = [['class', 'mAP'] + [f'AP@{e}' for e in thresholds] +
    ...
    for name in class_names:
        ...
        AP = ([name, mAP] + [round(ap * 100, 3) for ap in ap_list] +
